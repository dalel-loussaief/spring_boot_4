...
public class SecurityConfig  extends WebSecurityConfigurerAdapter {
	...
	auth.inMemoryAuthentication().withUser("user1").password(passwordEncoder.encode("123")).roles("USER");*/
	 ...
	 protected void configure(HttpSecurity http) throws Exception {
		 ...
		 http.authorizeRequests().antMatchers("/ListeVetement").hasAnyRole("ADMIN","AGENT","USER");
		 http.authorizeRequests().antMatchers("/supprimerVetement","/modifierVetement","/updateVetement").hasAnyRole("ADMIN");
